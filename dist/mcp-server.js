...
function createMcp(dbPath, rawToken) {
    ...
    if (!masterPw) {
        console.error('❌  VERYKEY_MASTER_PASSWORD env required');
        process.exit(1);
    }
    ...
    function handleToolCall(name, args) {
        switch (name) {
            case 'verykey_get': {
                ...
                if (!ref)
                    return { error: { code: -32602, message: 'Secret not found' } };
            ...
            case 'verykey_list': {
                ...
                secrets = secrets.filter((s) => token.level >= s.level);
                return {
                    result: {
                        content: [{
                                type: 'text',
            ...
            case 'verykey_search': {
                ...
                // 过滤权限
                const filtered = secrets.filter((s) => token.level >= s.level);
                return {
                    result: {
                        content: [{
                                type: 'text',
...
async function startMcpServer(dbPath, rawToken, port) {
...
function startMcpStdio(dbPath, rawToken) {
    const { token, handleRequest } = createMcp(dbPath, rawToken);
    console.error(`🔑  VeryKey MCP Server (stdio)`);
    console.error(`   Token: ${token.name} (L${token.level} - ${types_1.LEVEL_NAMES[token.level]})`);
